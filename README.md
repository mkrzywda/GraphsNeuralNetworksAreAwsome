![Twitter Follow](https://img.shields.io/twitter/follow/mackrzywda?style=social)
![GitHub Repo stars](https://img.shields.io/github/stars/mkrzywda/GraphsAreAwsome?style=social)
![GitHub watchers](https://img.shields.io/github/watchers/mkrzywda/GraphsAreAwsome?style=social)
![GitHub](https://img.shields.io/github/license/mkrzywda/GraphsAreAwsome?style=flat-square)

# Graphs are Awesome!

Hello!

My name is Maciej Krzywda. I created this repository to share a set of links that I found valuable and inspiring. 
I hope that you'll like it!

So sum up **Graphs are Awsome** is a space in you will be can find articles/sample of code about **_Graphs_** in Artificial intelligence (mostly Neural Networks)


## Table of Contents
1. [GNN Introduction](#gnn-introduction)
2. [Graph Neural Networks Optimization](#graph-neural-networks-optimization)
3. [Bayesan NN](#bayesan-nn)
4. [Capsule NN](#capsule-nn)
5. [No Categorized](#no-categorized)

## GNN Introduction


* [A Comprehensive Survey on Graph Neural Networks](https://arxiv.org/pdf/1901.00596.pdf)
* [Adversarial Attack and Defense on Graph Data: A Survey](https://arxiv.org/pdf/1812.10528.pdf)
* [Geometric deep learning: going beyond euclidean data](https://arxiv.org/pdf/1611.08097.pdf)
* [Relational inductive biases, deep learning, and graph networks](https://arxiv.org/pdf/1806.01261.pdf)
* [Attention models in graphs](https://arxiv.org/pdf/1807.07984.pdf)
* [Deep learning on graphs: A survey](https://arxiv.org/pdf/1812.04202.pdf)
* [Graph Neural Networks: A Review of Methods and Applications](https://arxiv.org/pdf/1812.08434.pdf)
* [Supervised neural networks for the classification of structures](https://www.ncbi.nlm.nih.gov/pubmed/18255672)
* [A new model for learning in graph domains.](https://ieeexplore.ieee.org/abstract/document/1555942)
* [The graph neural network model.](http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.1015.7227&rep=rep1&type=pdf)
* [Graph echo state networks.](https://ieeexplore.ieee.org/abstract/document/5596796)
* [Gated graph sequence neural networks.](https://arxiv.org/pdf/1511.05493.pdf)
* [Learning steady-states of iterative algorithms over graphs.](http://proceedings.mlr.press/v80/dai18a/dai18a.pdf)[
* [Spectral networks and locally connected networks on graphs.](https://arxiv.org/pdf/1312.6203.pdf)
* [Deep convolutional networks on graph-structured data.](https://arxiv.org/abs/1506.05163) 
* [Accelerated filtering on graphs using lanczos method.](https://arxiv.org/pdf/1509.04537.pdf)
* [Convolutional neural networks on graphs with fast localized spectral filtering.](https://arxiv.org/pdf/1606.09375.pdf)
* [Semi-supervised classification with graph convolutional networks.](https://arxiv.org/pdf/1609.02907.pdf)
* [Cayleynets: graph convolutional neural networks with complex rational spectral filters.](https://arxiv.org/pdf/1705.07664.pdf)
* [Simplifying Graph Convolutional Networks](https://arxiv.org/pdf/1902.07153.pdf)
* [Graph Wavelet Neural Network](https://openreview.net/pdf?id=H1ewdiR5tQ)



## Graph Neural Networks Optimization

* [Enabling Automated FPGA Accelerator Optimization Using Graph Neural Networks](https://arxiv.org/pdf/2111.08848)
* [Optimizing Sparse Matrix Multiplications for Graph Neural Networks](https://arxiv.org/pdf/2111.00352)
* [Learning General Optimal Policies with Graph Neural Networks: Expressive Power, Transparency, and Limits](https://arxiv.org/pdf/2109.10129)
* [Combinatorial Optimization with Physics-Inspired Graph Neural Networks ](https://arxiv.org/pdf/2107.01188)
* [Optimization of Graph Neural Networks: Implicit Acceleration by Skip Connections and More Depth](https://arxiv.org/pdf/2105.04550)
* [Reinforcement Learning for Scalable Logic Optimization with Graph Neural Networks](https://arxiv.org/pdf/2105.01755)
* [Optimizing Memory Efficiency of Graph Neural Networks on Edge Computing Platforms](https://arxiv.org/pdf/2104.03058)
* [Towards Deepening Graph Neural Networks: A GNTK-based Optimization Perspective](https://arxiv.org/pdf/2103.03113)
* [A Biased Graph Neural Network Sampler with Near-Optimal Regret](https://arxiv.org/pdf/2103.01089)
* [Combinatorial optimization and reasoning with graph neural networks](https://arxiv.org/pdf/2102.09544)
* [Interpreting and Unifying Graph Neural Networks with An Optimization Framework](https://arxiv.org/pdf/2101.11859)
* [Optimizing Large-Scale Fleet Management on a Road Network using Multi-Agent Deep Reinforcement Learning with Graph Neural Network](https://arxiv.org/pdf/2011.06175)
* [Optimization of Graph Neural Networks with Natural Gradient Descent](https://arxiv.org/pdf/2008.09624)
* [Policy-GNN: Aggregation Optimization for Graph Neural Networks](https://arxiv.org/pdf/2006.15097)
* [Optimization and Generalization Analysis of Transduction through Gradient Boosting and Application to Multi-scale Graph Neural Networks](https://arxiv.org/pdf/2006.08550)
* [Optimal Transport Graph Neural Networks ](https://arxiv.org/pdf/2006.04804)
* [Wireless Power Control via Counterfactual Optimization of Graph Neural Networks](https://arxiv.org/pdf/2002.07631)
* [Optimal Power Flow Using Graph Neural Networks ](https://arxiv.org/pdf/1910.09658)
* [Deep Reinforcement Learning meets Graph Neural Networks: exploring a routing optimization use case](https://arxiv.org/pdf/1910.07421)
* [RouteNet: Leveraging Graph Neural Networks for network modeling and optimization in SDN](https://arxiv.org/pdf/1910.01508)
* [Optimal Wireless Resource Allocation with Random Edge Graph Neural Networks](https://arxiv.org/pdf/1909.01865)
* [Topology Attack and Defense for Graph Neural Networks: An Optimization Perspective](https://arxiv.org/pdf/1906.04214)
* [Exact Combinatorial Optimization with Graph Convolutional Neural Networks](https://arxiv.org/pdf/1906.01629)
* [Unveiling the potential of Graph Neural Networks for network modeling and optimization in SDN](https://arxiv.org/pdf/1901.08113)
* [Training Deep Neural Networks via Optimization Over Graphs](https://arxiv.org/pdf/1702.03380)

## Capsule NN

* [Graph capsule convolutional neural networks.](https://arxiv.org/abs/1805.08090)
* [Capsule Graph Neural Network.](https://openreview.net/pdf?id=Byl8BnRcYm)

## Bayesian GNN

* [Bayesian Semi-supervised Learning with Graph Gaussian Processes.](https://papers.nips.cc/paper/7440-bayesian-semi-supervised-learning-with-graph-gaussian-processes.pdf)
* [Bayesian Graph Convolutional Neural Networks for Semi-supervised Classification.](https://arxiv.org/pdf/1811.11103.pdf)

## No Categorized

* [Spectral networks and locally connected networks on graphs.*](https://arxiv.org/pdf/1312.6203.pdf)
* [Deep convolutional networks on graph-structured data.](https://arxiv.org/abs/1506.05163) 
* [Accelerated filtering on graphs using lanczos method.](https://arxiv.org/pdf/1509.04537.pdf)
* [Convolutional neural networks on graphs with fast localized spectral filtering.](https://arxiv.org/pdf/1606.09375.pdf)
* [Semi-supervised classification with graph convolutional networks.](https://arxiv.org/pdf/1609.02907.pdf)
* [Cayleynets: graph convolutional neural networks with complex rational spectral filters.](https://arxiv.org/pdf/1705.07664.pdf)
* [Simplifying Graph Convolutional Networks.](https://arxiv.org/pdf/1902.07153.pdf)
* [Graph Wavelet Neural Network.](https://openreview.net/pdf?id=H1ewdiR5tQ)
* [Diffusion Scattering Transforms on graphs.](https://arxiv.org/pdf/1806.08829.pdf)
* [Neural network for graphs: A contextual constructive approach.](https://ieeexplore.ieee.org/abstract/document/4773279)
* [Convolutional networks on graphs for learning molecular fingerprints.](http://papers.nips.cc/paper/5954-convolutional-networks-on-graphs-for-learning-molecular-fingerprints.pdf)
* [Diffusion-convolutional neural networks](https://arxiv.org/pdf/1511.02136.pdf)
* [Neural message passing for quantum chemistry.](https://arxiv.org/pdf/1704.01212.pdf)
* [Dynamic Edge-Conditioned Filters in Convolutional Neural Networks on Graphs.](https://arxiv.org/pdf/1704.02901.pdf) 
* [Geometric deep learning on graphs and manifolds using mixture model cnns.](https://arxiv.org/pdf/1611.08402.pdf)
* [Robust spatial filtering with graph convolutional neural networks.](https://arxiv.org/abs/1703.00792)
* [Structure-Aware Convolutional Neural Networks.](http://papers.nips.cc/paper/7287-structure-aware-convolutional-neural-networks.pdf)
* [On filter size in graph convolutional network.](https://arxiv.org/pdf/1811.10435.pdf)
* [Predict then Propagate: Graph Neural Networks meet Personalized PageRank.](https://arxiv.org/pdf/1810.05997.pdf)
* [Representation learning on graphs with jumping knowledge networks.](https://arxiv.org/pdf/1806.03536.pdf)
* [Dual graph convolutional networks for graph-based semi-supervised classification.](https://dl.acm.org/citation.cfm?id=3186116)
* [Graph U-nets.](https://arxiv.org/pdf/1905.05178.pdf)
* [MixHop: Higher-Order Graph Convolutional Architectures via Sparsified Neighborhood Mixing.](https://arxiv.org/pdf/1905.00067.pdf)
* [Graph Attention Networks.](https://arxiv.org/pdf/1710.10903.pdf)
* [Gaan: Gated attention networks for learning on large and spatiotemporal graphs.](https://arxiv.org/pdf/1803.07294.pdf)
* [Geniepath: Graph neural networks with adaptive receptive paths.r](https://arxiv.org/pdf/1802.00910.pdf)
* [Graph Representation Learning via Hard and Channel-Wise Attention Networks.](https://www.kdd.org/kdd2019/accepted-papers/view/graph-representation-learning-via-hard-and-channel-wise-attention-networks) 
* [Understanding Attention and Generalization in Graph Neural Networks.](https://arxiv.org/abs/1905.02850)
* [Learning convolutional neural networks for graphs.](https://arxiv.org/pdf/1605.05273.pdf)
* [Large-Scale Learnable Graph Convolutional Networks.](https://arxiv.org/pdf/1808.03965.pdf)
* [Inductive representation learning on large graphs](http://papers.nips.cc/paper/6703-inductive-representation-learning-on-large-graphs.pdf)
* [Stochastic Training of Graph Convolutional Networks with Variance Reduction.](https://arxiv.org/pdf/1710.10568.pdf)
* [Adaptive Sampling Towards Fast Graph Representation Learning.](https://arxiv.org/pdf/1809.05343.pdf)
* [FastGCN: Fast Learning with Graph Convolutional Networks via Importance Sampling.](https://arxiv.org/pdf/1801.10247.pdf)
* [Cluster-GCN: An Efficient Algorithm for Training Deep and Large Graph Convolutional Networks.](https://arxiv.org/pdf/1905.07953.pdf)
* [Hierarchical graph representation learning with differentiable pooling.](https://arxiv.org/pdf/1806.08804.pdf)
* [Self-Attention Graph Pooling.](https://arxiv.org/abs/1904.08082)
* [Contextual graph markov model: A deep and generative approach to graph processing.](https://arxiv.org/abs/1805.10636)
* [Adaptive graph convolutional neural networks.](https://arxiv.org/pdf/1801.03226.pdf)
